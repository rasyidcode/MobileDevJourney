package com.rasyidcode.forageapp.data

import androidx.room.Dao

/**
 * Data Access Object for database interaction
 */
@Dao
interface ForageableDao {

    // TODO: implement a method to retrieve all Forageables from the database

    // TODO: implement a method to retrieve a Foregable from the database by id

    // TODO: implement a method to insert a Foregeable into the database
    //  (use OnConflictStrategy.REPLACE)

    // TODO: implement a method to update a Foregeable that is already in the database

    // TODO: implement a method to delete a Forageable from the database

}